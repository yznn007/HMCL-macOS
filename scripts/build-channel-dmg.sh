...
HMCL_CHANNEL="$(printf '%s\n' "$DOWNLOAD_OUTPUT" | awk -F= '/^HMCL_CHANNEL=/{print $2}')"

if [ -z "$HMCL_JAR" ] || [ -z "$HMCL_TAG" ] || [ -z "$HMCL_CHANNEL" ]; then
  printf 'Error: failed to resolve HMCL jar, tag, or channel from download output\n' >&2
  exit 1
fi
